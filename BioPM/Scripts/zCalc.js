...
function calc_z() {
    if (Math.abs(document.calc2.z.value) > Z_MAX) {
        ...
        document.calc1.q.value = "*Error*";
    } else {
        var qz = 1 - poz(Math.abs(document.calc1.z.value));

        document.calc1.q.value = trimfloat(qz, ROUND_FLOAT);
        document.calc1.onein.value = Math.round(1 / qz);
    }
}
...
function calc_q() {
    if (document.calc2.q.value < 0 ||
        document.calc2.q.value > 1) {
        alert("Probability (Q) must be between 0 and 1.");
        document.calc2.z.value = "*Error*";
    } else {
        document.calc2.z.value = trimfloat(Math.abs(critz(document.calc2.q.value)), ROUND_FLOAT);
    }
}
